...
export const Particles: React.FC<ParticlesProps> = ({
  ...
  const animate = () => {
    ...
    circles.current.forEach((circle: Circle, i: number) => {
      ...
      // Local repel effect
      const px = circle.x + circle.translateX - canvasSize.current.w / 2
      const py = circle.y + circle.translateY - canvasSize.current.h / 2
      const dxMouse = px - mouse.current.x
      const dyMouse = py - mouse.current.y
      const dist = Math.sqrt(dxMouse * dxMouse + dyMouse * dyMouse)
      const repelRadius = 160
      
      if (dist < repelRadius && dist > 0) {
        const force = (repelRadius - dist) / repelRadius
        circle.translateX += (dxMouse / dist) * force * 8
        circle.translateY += (dyMouse / dist) * force * 8
      }
